...
class Engine:
    INF = 99999999999
    NEG_INF = -99999999999
    ML_N = 2  # this constant is used to tune the usage of machine learning

    def __init__(self, fen):
        self.board = chess.Board()
        self.model = load_model("fBIGelo1")
        ##########################
        # Values of pieces
        ##########################
        self.pieces = {
    ...
    def alphabeta(self, depth, mx_depth, alpha, beta, last_move=""):
        ...
        if mx_depth - depth <= self.ML_N:
            ...
            for move in legal_moves:
                self.board.push(move)
                lst.append([self.ml_elal(), move])
                self.board.pop()
            if maxormin:  # that's a white move, and we are interested in maximizing evaluation
    ...
    # This function transforms a chess position to an input for a Machine Learning model
    def make_a_row(self):
        lst = [0 for i in range(2 * 6 * 8 * 8)]
        half = 384
        for i in range(1, 7):
            for x in self.board.pieces(i, False):
                lst[0 + (i - 1) * 64 + x] = 1
            for x in self.board.pieces(i, True):
                lst[half + (i - 1) * 64 + x] = 1
